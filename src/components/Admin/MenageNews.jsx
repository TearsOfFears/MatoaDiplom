...
const mapState = ({ productsData, newsData }) => ({
	products: productsData.products,
	news: newsData.newsRender.dataNews,
});
const MenageProducts = () => {
	const [hideModal, setHideModal] = useState(false);
	const { products, news } = useSelector(mapState);
	...
	const deleteAll = (deleteLinks, documentID) => {
		...
		dispatch(deleteNews(documentID));
	...
	const handleGetContentNews = (documentID) => {
		dispatch(fetchNewsEdit(documentID));
		toggleModal();
	};
	const configModal = {
		hideModal,
		...
		<div className="menageProducts">
							...
							news.map((data, pos) => {
								...
								return (
									<TableRow key={documentID} style={styles}>
										...
										<TableCell align="left">{newsLink}</TableCell>
										<TableCell align="left">
											<button
												className="delete"
												onClick={() => handleGetContentNews(documentID)}
											>
												<svg
			...
			c2.518,2.51,3.905,5.855,3.905,9.414C470.516,51.036,469.127,54.38,466.61,56.897z"
										...
										<TableCell align="left">
											<button
												className="delete"
												onClick={() => deleteAll(newsThumbnail, documentID)}
											>
												<svg
													width="17"
													height="19"
